...
param(
    ...
    [string]$OutDir = "build\linux-net-to-hdmi-direct-copy"
)
...
function Stop-StaleProcesses {
    $stale = Get-CimInstance Win32_Process |
        Where-Object {
            ...
            ($_.CommandLine -match "send_unified_test_video_udp\.py" -or
...
try {
    ...
    $receiverTargetFrames = $Frames + $WarmupFrames
    ...
    @(
        ...
        "/tmp/fb_video_udp_receiver --port $UdpPort --frames $receiverTargetFrames --timeout-sec 180 --control-fifo $ControlFifo --sync-mode $ReceiverSyncMode --fb-copy-mode direct-memcpy --present-fps $ReceiverPresentFps > /tmp/fb_video_udp_receiver.log 2>&1 & echo RECEIVER_PID=`$!",
        "sleep 1",
    ...
    if ($deployLog -notmatch "/tmp/fb_video_udp_receiver: OK" -or
        ...
        $deployLog -notmatch "FB_COPY_MODE mode=direct-memcpy" -or
        $deployLog -notmatch "VIDEO_UDP_LINUX_RECEIVER_READY .*fb_copy_mode=direct-memcpy") {
        throw "receiver deploy/start direct-copy markers missing"
    }
    ...
    if (-not $dashboardReady) {
        throw "dashboard capture endpoint did not become ready"
    }
    ...
    & python (Join-Path $repoRoot "tools\send_unified_test_video_udp.py") `
        ...
        --hold-repeats 1 `
        --content-hold-frames 1 `
        --wire-format fb24-native `
        --out-dir $senderOut `
    ...
    if (-not $mjpegProcess.WaitForExit(60000)) {
    ...
    $afterCommands = Join-Path $outPath "uart_after_direct_copy.commands"
    @(
        "cat /tmp/fb_video_udp_receiver.log",
        "kill `$(pidof fb_video_udp_receiver 2>/dev/null) 2>/dev/null || true",
    ...
    & (Join-Path $repoRoot "tools\uart_run_commands.ps1") `
        ...
        -InterCommandDelayMilliseconds 500 `
        -FinalReadSeconds 2 `
        -OutputPath (Join-Path $OutDir "uart_after_direct_copy.log")

    $afterLog = Get-Content -Raw -LiteralPath (Join-Path $outPath "uart_after_direct_copy.log")
    if ($afterLog -notmatch "FB_COPY_MODE mode=direct-memcpy") {
        throw "final receiver log missing direct copy marker"
    }
    ...
    $senderTracePath = Join-Path $senderOut "sender-trace.json"
    ...
    $wireFormat = [string]$senderJson.wire_format
    ...
    $summaryPath = Join-Path $outPath "linux-net-to-hdmi-direct-copy-summary.json"
    $summary | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $summaryPath -Encoding UTF8

    $markerBits = "receiver_fb_copy_mode=direct-memcpy sender_wire_format=$wireFormat sender_fps=$senderFps sent_frames=$sentFrames receiver_written_frames=$receiverFrames receiver_dropped_packets=$receiverDropped receiver_effect=none mjpeg_saved_frames=$mjpegSavedFrames mjpeg_unique_hashes=$mjpegUniqueHashes mjpeg_unique_colors=$mjpegUniqueColors trace_require_image_paths=$traceRequireImages trace_image_path_failures=$($traceMetrics.image_path_failures) validator_status=$validatorStatus trace_sent_frames=$($traceMetrics.sent_frames) trace_matched_frames=$($traceMetrics.matched_frames) trace_drop_rate=$($traceMetrics.drop_rate) trace_order_violations=$($traceMetrics.order_violations) trace_content_mismatches=$($traceMetrics.content_mismatches) trace_black_frames=$($traceMetrics.black_frames) trace_required_max_latency_ms=$($validator.requirements.max_latency_ms) trace_max_latency_ms=$($traceMetrics.max_latency_ms) sent_time_offset_ms=$sentTimeOffsetMs out=$outPath"
    if ($passCondition) {
        $marker = "LINUX_NET_TO_HDMI_DIRECT_COPY_OK $markerBits"
        Set-Content -LiteralPath (Join-Path $outPath "linux-net-to-hdmi-direct-copy.marker.txt") -Value $marker -Encoding ASCII
        Write-Host $marker
    } else {
        $marker = "LINUX_NET_TO_HDMI_DIRECT_COPY_FAIL $markerBits"
        Set-Content -LiteralPath (Join-Path $outPath "linux-net-to-hdmi-direct-copy.marker.txt") -Value $marker -Encoding ASCII
        throw $marker
    }
}
