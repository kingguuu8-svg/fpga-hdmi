...
try {
    ...
    @(
        ...
        "/tmp/fb_video_udp_receiver --port $UdpPort --frames $receiverFrames --timeout-sec $receiverTimeout --control-fifo $ControlFifo --sync-mode none --present-fps $ReceiverPresentFps > /tmp/fb_video_udp_receiver.log 2>&1 & echo RECEIVER_PID=`$!",
        "sleep 1",
