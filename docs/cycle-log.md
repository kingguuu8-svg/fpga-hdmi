...
Commit: this commit (`cycle: verify jpegpldec PL-returned writeback`)

Objective: prove that `jpegpldec` can write bytes returned from the
PS-to-PL-to-PS DMA path back into the downstream GstBuffer without changing
the external RTP/JPEG GStreamer chain.

Changed scope:

- Added `probe-mode=dma-writeback`.
- Added staging-buffer marker stamping before DMA and GstBuffer copy-back from
  the coherent RX result.
- Extended the connected-board runner with a .NET file server, PL probe-core
  reset before precheck, and HDMI marker validation for writeback mode.
- Updated the `jpegpldec` README and pipeline skill.

Verification performed:

- `jpegpldec` cross-build passed.
- Python validator compile and PowerShell parser checks passed.
- Standalone board DMA precheck passed at 115200 bytes.
- `jpegpldec` processed 60 real decoded frames in `dma-writeback` mode.
- PL counters recorded 480 transactions and 6,912,000 bytes.
- HDMI dynamic validation passed with 300 samples, 104 unique hashes, and
  264.944 pixels of detected motion.
- HDMI writeback-marker validation passed with 224/300 frames.

Board action:

- Loaded the module and plugin from `/tmp`, ran the video writeback probe, and
  stopped the receiver. No boot image or nonvolatile storage changed.

Evidence:

- `docs/reports/jpegpldec-pl-returned-buffer-writeback.md`
...
Result: PASSED. PL-returned raw-frame bytes are now reconnected to downstream
GStreamer through a copy-back GstBuffer writeback path.
...
- The path is not zero-copy.
- The visible marker is stamped in the plugin staging copy before DMA, not
  generated by PL logic.
- JPEG entropy decode is still the software `jpegdec` child.
